...
class UserInterfaceBoard:
    ...
    def mouse_up_left(self, y, x):
        ...
        # Check if this is a real human click (dragged off tile)
        if self._click and (self._click[0] != y or self._click[1] != x):
            y, x = self._click
            # Reset tile visual if it was a "pressed" state
        ...
        # This is the main logic for both AI (no mouse down) and human clicks (mouse up on same tile)
        else:
            tile_was_mine = False

            # This is the tile's current visual state
            current_visual_tile = self._tiles[y][x]

            # Check if the tile is logically unopened AND
            # visually is either 'empty' (human click) or 'unopened' (AI click)
            if (
                self._core._tiles[y][x] == core.BoardTile.unopened
                and (
                    current_visual_tile == self._sprites.empty
                    or current_visual_tile == self._sprites.unopened
                )  # <-- THE FIX
            ):
            ...
            elif current_visual_tile == self._sprites.question_mark_click:
                self._tiles[y][x] = self._sprites.unopened
